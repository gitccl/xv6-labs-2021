...
e1000_transmit(struct mbuf *m)
{
  ...
  uint32 txindex = regs[E1000_TDT];
  struct tx_desc *tx = &tx_ring[txindex];
  if(!(tx->status & E1000_TXD_STAT_DD)) {
    printf("e1000_transmit: E1000_TXD_STAT_DD is not set\n");
    release(&e1000_lock);
    return -1;
  }

  if(tx_mbufs[txindex]) {
    mbuffree(tx_mbufs[txindex]);
    tx_mbufs[txindex] = 0;
  }
  ...
  regs[E1000_TDT] = (txindex + 1) % TX_RING_SIZE;
  release(&e1000_lock);
  return 0;
}
